...
-- От общего числа игроков 22214, платящих игроков 3929 это 17,69%. 
-- При рассмотрении расы отмечу топ 3 из них: Demon - 19.37%, Hobbit - 18.06% и Human - 17.6%. По Human отдельно отмечу, что не смотря на третье место по проценту платящих игроков, по количеству платящих игроков занимает первое место – 1114 и также первое место по общему количеству игроков этой расы - 6328.
...
-- Общее количество покупок - 1 307 678 в общей сложности на сумму 686 615 040; минимальная сумма составляет 0, максимальная сумма транзакции составляет 486615.1; среднее значение составляется 525.69, а медиана 74.86, то есть между ними существенная разница; разброс данных составляет 2517.35.
-- 1.3. Есть ли аномальные покупки по стоимости? Если есть, то сколько их?
-- Найдены покупки с нулевой стоимостью в количестве 907 штук. В процентном соотношении составляет всего 0.0007%, что не влияет критически на статистику по стоимости, но может оказывать влияние на абсолютное количество игроков, совершивших покупку.
-- 1.4. Есть ли среди эпических предметов популярные, которые покупают чаще всего? 
-- Среди всех предметов сильно выделяется Book of Legends с общими продажами 1004516, что составляет 0.77 доли от общих продаж; купили его 12195 раз с долей 0.55. 
-- На втором месте Bag of Holding с показателем 271875, доля продаж 0.21; купили его 11968 с долей 0.54
...
-- Согласно данных среднее количество покупок больше всего у Human (121.41) и Angel (106.82) и что связано с тем, что средняя стоимость одной покупки в сравнении с другими расами сильно ниже. Из этого можно сделать вывод, что гипотеза не подтверждена, ведь за Human и Angel стоимость эпических предметов значительно ниже, что облегчает покупку эпических предметов, а следовательно, и игровой.
-- 3. Общие выводы и рекомендации
-- Стоит изменить стоимость предметов чтобы все расы получали одинаковую возможность в покупке эпических предметов, а следовательно, одинаковую сложность игры.
-- В game_items следует подробно изучить предмет Book of Legends. При анализе обнаружено, что этот предмет может стоить как 0, так и 486615.1 в валюте. Стоит подробно изучить при каких обстоятельствах это происходит и исключить из игры возможность покупки этого предмета на 0.
